...
def calc_root_mean_squared_error(y_N, yhat_N):
    ...
    # making sure the right specs of input parameters
    assert y_N.ndim == 1
    assert y_N.shape == yhat_N.shape

    # calculate mean squared difference (absolute value)
    # msd = np.square(np.subtract(y_N, yhat_N)).mean() # backup plan
    msd = np.absolute(np.mean((y_N - yhat_N) ** 2))
    # calculate root mean squared difference
    rmse = np.sqrt(msd)
    return rmse  # TODO fixme
