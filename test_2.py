...
my_bow_classifier_pipeline = sklearn.pipeline.Pipeline([
    ('my_bow_feature_extractor', CountVectorizer(min_df=4, max_df=1.0, ngram_range=(1, 1),
                                                 vocabulary=vocab_dict, binary=False)),
    ('my_classifier', sklearn.linear_model.LogisticRegression(C=10000.0, max_iter=100, random_state=101)),
    # ('my_splitter', sklearn.model_selection.PredefinedSplit(valid_indicators_N)),
    # ('grid_searcher', sklearn.model_selection.GridSearchCV(np.logspace(-4, 4, 9), [1, 2, 3], refit=False))
])
