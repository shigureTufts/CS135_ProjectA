...
my_bow_classifier_pipeline = sklearn.pipeline.Pipeline([
    ('my_bow_feature_extractor',
     CountVectorizer(min_df=1, max_df=1, ngram_range=(1, 1), vocabulary=vocab_dict, binary=False)),
...
my_parameter_grid_by_name['my_bow_feature_extractor__binary'] = [True, False]
my_parameter_grid_by_name['my_classifier__C'] = np.logspace(-6, 6, 101)
# my_parameter_grid_by_name['my_bow_feature_extractor__max_df'] = [0.5, 0.7, 0.9, 1.0]
my_parameter_grid_by_name['my_bow_feature_extractor__ngram_range'] = [(1,1), (1,2)]
...
param_keys = ['param_my_bow_feature_extractor__binary', 'param_my_bow_feature_extractor__ngram_range', 'param_my_classifier__C']
...
new_pipeline = sklearn.pipeline.Pipeline([
    ('my_bow_feature_extractor', CountVectorizer(min_df=1, max_df=1.0, ngram_range=(1, 2),
                                                 vocabulary=vocab_dict, binary=True)),
    ('my_classifier', sklearn.linear_model.LogisticRegression(C=1.7378008287493762, max_iter=1000,
                                                              penalty='l2', solver='lbfgs'))
])
...
my_parameter_rand_by_name['my_classifier__C'] = np.random.uniform(1.0, 10, 10000)
my_parameter_rand_by_name['my_classifier__penalty'] = ['l2']
...
param_keys = ['param_my_classifier__penalty', 'param_my_classifier__C']

# Rearrange row order so it is easy to skim
rand_search_results_df.sort_values(param_keys, inplace=True)

var_2 = rand_search_results_df[param_keys + ['mean_test_score', 'rank_test_score']]
